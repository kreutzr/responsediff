  ...
  throws JsonMappingException, JsonProcessingException
  {
    ...
    if( xmlResponse != null && xmlResponse.getExpected() != null && !checkOnlyUnexpected ) {
      ...
      if( xmlExpected.getHeaders() != null ) {
        ...
        final String headersAsJson = ToJson.fromXmlHeaders( candidateResponse.getHeaders(), true, lowerCaseHeaderNamesWithPaths ); // All "real" headers with their values (which may be JSON)
    ...
    relevantDiffs.join( validateJson(
      ToJson.fromXmlHeaders( candidateResponse.getHeaders(), true, null ),
      ToJson.fromXmlHeaders( referenceResponse.getHeaders(), true, null ),
      headerIgnore,
      reportWhiteNoise,
      testId
    ) );
