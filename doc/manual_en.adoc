= ResponseDiff - Version 1.5.1-SNAPSHOT
:encoding: utf-8
:lang: de
:toc: left
:toclevels: 5
...
**Script-Based HTTP Workflows**

If the comparison mode is disabled, ResponseDiff and its setup can also be used to process successive server requests. Parameters defined in a script may be passed as variables, making the use very flexible. ResponseDiff can therefore be used as a simple and cost-effective alternative to commercial, UI-based tools like Postman or Bruno. Further information about this mode can be found in the <<Configuration>> section under the parameters `initialVariables` and `compareResponses`.
...
=== Java
ResponseDiff requires Java version 17 or higher.
...
.JSON for the call configuration
====
[source,json]
----
{
  ...
  "epsilon" : ...,
  "initialVariables" : [ { "id" : "...", "value" : "..." }, ... ],
  "compareResponses" : [true|false],
...
* `ticketServiceUrl`: Optional URL to the ticket system, so that the ticket number only needs to be appended (e.g. `http://localhost/tickets/`). This parameter is *deprecated* and will be removed in version 2.0. It is replaced by the parameter `ticketServiceUrls`.
* `ticketServiceUrls`: Optional, comma-separated list of the ticket system URLs to be used, so that the ticket number only needs to be appended (e.g., `http://localhost/tickets/`). If more than one URL is used, the URLs must be distinguished by an identifier (e.g., `ticketServiceUrls:"repo1=http://localhost/tickets/repo1/,repo2=http://localhost/tickets/repo2,old=http://localhost/legacy-ticketsystem/"`). If no identifier is specified for a URL, the identifier `default` is used internally (e.g., `ticketServiceUrls:"new=http://localhost/new-ticketsystem/,http://localhost/legacy-ticketsystem/"`). The identifiers can be used in the test definitions to reference the ticketing system to be used (e.g., `ticketReference="new:123"`). If no identifier is specified in a `ticketReference`, the identifier `default` is assumed to be meant (e.g., `ticketReference="456"`).
...
* `useLogo`: Optional Flag, if a logo shall be used (`true`) when creating a report or not (`false`). (default is `false`)
...
* `maskAuthorizationHeaderInCurl`: Optional Flag indicating whether values of the `Authorization` header shall be masked (`true`) or not (`false`). (default is `true`)
...
* `referenceHeaders`: Optional headers that should be used when calling the reference instance. (e.g. `[ { "name" : "Authorization", "value" : "bearer ..." } ]`)
* `controlHeaders`: Optional headers that should be used when calling the control instance. (e.g. `[ { "name" : "Authorization", "value" : "bearer ..." } ]`)
...
* `initialVariables`: Optional list of variables to be included in the test setup (see parameter `xmlFilePath`). Existing setup variables with the same `id` will be overwritten for the test run. The variables passed here are marked with the attribute `configured` (value `true`) in the generated XML file.
* `compareResponses`: Optional flag to determine whether server responses should be compared (`true`) or not (`false`). (default is `true`)
...
* `warn`: The test will be included in the report if a warning occurred.
...
* `orga`: Tests and test sets are excluded by default if the `orga` attribute of the respective test or test set is set to `true` (the default is `false`). This can be useful if the report should only show the actual tests, but not the setup and cleanup of the test scenario that might be necessary. Tests and test sets marked with `orga` are included in the report only if the `report` attribute also contains `orga`.
...
The HTML report can be customized as described in the section <<Customized Report>>.
...
The PDF report can be customized as described in the section <<Customized Report>>.
...
.XML for embedding external test setups
====
[source,xml]
----
        ...
...
To prevent a test sequence from being aborted "only" because a relatively unimportant test (e.g., a timing measurement) fails, a log level (`logLevel`) can be defined for each value check. Only the levels `ERROR` and `FATAL` will lead to a test termination, but not, for example, `WARN`. If no LogLevel is explicitly specified, `ERROR` is assumed as the default.
...
.Test abort definition and `logLevel` usage
====
[source]
----
<XmlResponseDiffSetup ...>
...
----
<1> The enclosing TestSet defines a sequential execution of its inner elements by specifying `order="strict"`. This ensures that the TestSet with ID "before" is executed first.
<2> The TestSet "before" is marked with `breakOnFailure="true"`. If execution fails with only one (inner) error (`ERROR` or `FATAL`), the subsequent TestSets "test" and "after" are skipped ("local test termination"). If the execution order of the inner tests were relevant, `order="strict"` would also have to be set here.
<3> The test with ID "Create test element" is also marked with `breakOnFailure="true"`. A failure of this test results in all subsequent tests being skipped.
<4> A LogLevel can optionally be specified for the expected values. If the LogLevel is not `ERROR` or `FATAL`, a failure will **not** trigger a local test termination.
<5> The same applies to the overall expected values ​​(`overAllExpected`).
====
...
----
[
  { "id" : "002", "a" : [ { "x" : 3, "y" : 1   }, { "x" : 2, "y" :  1   } ], "b" : [ 6,5,4 ], "c": [ 9,8,7 ], "x": [ [ 3,2,1 ], [ [ 6,5,4 ], [ 9,8,7 ] ] ] },
  ...
  <filterRegistry>
    <filter id="sortJsonBody" clazz="com.github.kreutzr.responsediff.filter.response.SortJsonBodyResponseFilter">
      <parameters>
         ...
         <parameter id="sortArrays.keys">$($.id),a($.x;$.y),b,x[1][0]</parameter>
      </parameters>
    </filter>
    ...
----
...
[
  ...
  { "a" : [ { "x" : 2, "y" : 1   }, { "x" : 3, "y" :  1   } ], "b" : [ 4,5,6 ], "c": [ 9,8,7 ], "id" : "002", "x" : [ [ 3,2,1 ],[ [ 4,5,6 ], [ 9,8,7 ] ] ] }
]
----
...
* For list `x` only the element `x[1][0]` is sorted.
...
[CAUTION]
====
If the query contains a custom *sorting criterion* (e.g., `sort=name`), care must be taken to ensure that the configuration of the `SortJsonBodyResponseFilter` does not interfere with the desired sorting. In such cases, it is often advisable to suppress filter inheritance (see the `inherit` attribute of the `filters` element in the <<Response>> section).
...
.Expected values ​​in functional tests
====
[source,xml]
----
     <test>
...
[CAUTION]
====
ResponseDiff does not allow overwriting already defined variables with response values, as this would unnecessarily complicate the traceability of the calls and their parameters. If an overwrite attempt is detected, a warning message is issued and the original value is retained.
====
...
.Reading and using response values
====
[source,xml]
----
<XmlResponseDiffSetup ...>
...
.Configuration for the SetVariablesRequestFilter (The result is identical for both alternatives.)
====
----
{
"variables" : {
...
.Configuration with structured variables
====
[source,json]
----
{
...
.Multiple execution of a test set and a test
====
[source,xml]
----
...
.Proprietary JSONPath syntax extension
====
[source,xml]
----
  <variables>
...
.Avoiding Compressed Answers - Alternative 1
====
[source]
----
<request>
...
.Avoiding Compressed Answers - Alternative 2
====
[source]
----
<request>
...
[CAUTION]
If a test is not executed due to the `ifExecutionContextContains` attribute, then of course no variable values ​​can be read from a response.
...
=== Response Header with JSON Content

If a response header contains a JSON structure, its content can also be accessed for testing.

.The response header "x-duration"
====
In this example, a server-side performance measurement was implemented such that the time of receipt of a request and the time of dispatch of the response are recorded, the duration between these times is determined, and everything is returned as a JSON header.

Below, we see the value of such a measurement as a JSON structure and assume that it is returned as the `x-duration` header in the response.
[source]
----
{"begin":"2025-11-12T18:54:40.225345222Z","duration":"PT0.003391786S","end":"2025-11-12T18:54:40.228737008Z"}
----
====
For evaluation purposes, the `<header>` tag has the attribute `path`. This attribute is considered *exclusively* for `<header>` tags within a `<expected><headers>` tag structure. The value of the `path` attribute must be expressed as JSONPath. All options from the section section "<<Checking values ​​(`values`)>>" are available for testing.

.Testing of a JSON Response Header
====
[source]
----
<?xml version="1.0" encoding="UTF-8"?>
...
----
<1> We assume an execution time (measured server-side) of 5 milliseconds, allowing a tolerance of 3 milliseconds. Therefore, the maximum acceptable execution time is 8 milliseconds. This example does not assume that the execution time can be less than 2 milliseconds.
====

=== User specific Request Headers

Some tests or test sets may be tailored to specific users with particular roles. The technical identification of users is handled via the `Authorization` header, which is configured in the JSON provided when ResponseDiff is called using the keys `candidateHeaders`, `referenceHeaders`, and `controlHeaders`. To differentiate the `Authorization` headers of different users, the optional `userId` attribute must be used.

.Configuration of a user specific request
====
[source]
----
{ ...,
"candidateHeaders": [ { "name" : "Authorization", "value" : "bearer ...", "userId" : "PRIVILEGED_USER" }, { "name" : "Authorization", "value" : "bearer ...", "userId" : "RESTRICTED_USER" } ],
"referenceHeaders": [ { "name" : "Authorization", "value" : "bearer ...", "userId" : "PRIVILEGED_USER" }, { "name" : "Authorization", "value" : "bearer ...", "userId" : "RESTRICTED_USER" } ],
"controlHeaders": [ { "name" : "Authorization", "value" : "bearer ...", "userId" : "PRIVILEGED_USER" }, { "name" : "Authorization", "value" : "bearer ...", "userId" : "RESTRICTED_USER" } ],
... }
----
====
...
* This attribute `userId` is optional as long as the tests only apply to one user. As soon as another user is included in the tests, this attribute becomes mandatory.
* This attribute is used exclusively for request headers.
* The specified value is case-sensitive.
* The use of variables for this attribute is currently not supported.

To allow referencing specific users, tests and test sets also have an optional `userId` attribute. This information is inherited by encapsulated tests and test sets, but can also be overwritten.

If the `userId` attribute is set directly or through inheritance during a test, all headers without a specific `userId`, as well as all headers with a matching value, will be used in the query.


.Usage of a user specific request
====
[source]
----
<?xml version="1.0" encoding="UTF-8"?>
<XmlResponseDiffSetup id="...">
  <testSet id="..." userId="PRIVILEGED_USER"> <1>
    <test id="..."> <2>
      ...
    </test>
    <testSet id="..." userId="RESTRICTED_USER"> <3>
      <test id="...">
        ...
      </test>
    </testSet>

    <testSetInclude file="./included.xml" /> <4>
  </testSet>
</XmlResponseDiffSetup>
----
<1> For all tests, local and included TestSets, the user `PRIVILEGED_USER` is used.
<2> This test inherits the attribute `userId` with the value `PRIVILEGED_USER` from the enclosing structure.
<3> For this test set and the encapsulated tests, the attribute `userId` has been overridden.
<4> This test set inherits the `userId` with the value `PRIVILEGED_USER` from the enclosing structure.
====

=== Customized Report

It is often useful to customize the generated report to match your company's design. ResponseDiff offers the option to embed a logo and adjust colors and fonts. Furthermore, it is generally possible to customize the XSLT script that transforms the underlying XML report to AsciiDoc (`*.adoc`). Customizing colors and fonts is done in the second step, when the XML-transformed AsciiDoc report is converted into the target formats (e.g., HTML and PDF).

==== Embedding a Logo

For a logo to be displayed in the report,

* the logo as the file `logo.png` must be located directly next to the XSLT file (e.g., `report-to-adoc.xslt`) used for report generation (see `xsltFilePath` in the "<<Configuration>>" section) and
* the parameter `useLogo` (also see the "<<Configuration>>" section) must be passed as `true` when calling the script.

[NOTE]
For demonstration purposes, the file `logo.png` (the ResponseDiff logo) is included in the `reporter` directory upon delivery. The `useLogo` parameter is already passed in the prepared script `start-responseDiff`.

==== Customizing the Design for HTML

The report initially uses a default stylesheet. To customize colors and fonts, a file `custom-html-overrides.html` must be placed directly next to the XSLT file (e.g., `report-to-adoc.xslt`) used to generate the report (see `xsltFilePath` in the "<<Configuration>>" section). The file can contain any CSS selectors and declarations, all enclosed in a classic `<style>` HTML block. There are no restrictions on the CSS you can use.

[NOTE]
Upon delivery, the file `\_custom-html-overrides.html` is included in the `reporter` directory for demonstration purposes. It shows an example of how customization can be implemented using standard CSS syntax. Since the file is an HTML fragment that is inserted at the end of the `<head>` block during conversion, it is also possible, in principle, to place a `<script>` block within the file and embed custom JavaScript code.

==== Customizing the Design for PDF

The report initially uses a default theme. To customize colors and fonts, a file named `custom-pdf-theme.yml` must be placed directly next to the XSLT file (e.g., `report-to-adoc.xslt`) used for report generation (see `xsltFilePath` in the "<<Configuration>>" section). It is recommended to leave the line `extends: default` as is and only specify the properties that should be overridden. In principle, there are no restrictions regarding customization options.

[NOTE]
For demonstration purposes, the file `\_custom-pdf-theme.yml` is included in the `reporter` directory upon delivery. It shows an example of how to embed a PDF theme. The syntax strongly depends on the used 3rd part library AsciiDoctor-PDF.
...
[NOTE]
====
Since errors in the setup of the test scenario will lead to subsequent errors, it is worth familiarizing yourself with the `breakOnFailure` and `logLevel` attributes (see section "<<Local test abort>>").
====

.Local setup, test and teardown calls
====
[source]
----
<XmlResponseDiffSetup ...>
  <testSet id="Complex test" order="strict" report="success,fail,skip,warn,o_rga">
    <testSet id="before" breakOnFailure="true" orga="true"> <1>
      ...
    </testSet>
    <testSet id=test"> <2>
      ...
    </testSet>
    <testSet id=after" orga="true"> <3>
      ...
    </testSet>
  </testSet>
</XmlResponseDiffSetup>
----
<1> Setting the `breakOnFailure` attribute prevents subsequent errors during the actual test execution if this test set could not be executed successfully.
<2> The test sets with IDs `test` and `after` are marked as "skipped" in the report if the preparation test set `before` was unsuccessful.
<3> The test set is (as point 1) not included in the report in detail due to the `orga="true"` flag if no `orga` entry is found in the surrounding `report` attribute (see section `<<AsciiDoc (default)>>`).
====

If the setup file becomes too large, the "setup" (`before`) and "teardown" (`after`) test sets can be swapped out to separate files and included using the `testSetInclude` tag. Swapping out is also useful if the swapped out calls are to be used elsewhere. A desired `breakOnFailure` attribute must be set in the swapped out file when swapping out.
...
.Outsourced setup, test and teardown calls
====
[source]
----
<XmlResponseDiffSetup ...>
