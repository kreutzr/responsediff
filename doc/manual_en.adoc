...
The JSONPATH-Syntax does not allow dots in attribut names because these are reserved as path separators. It is also not possible mask dots (see section "<<Supported JSONPath syntax>>"). Using the <<NormalizeJsonBodyResponseFilter>> gives you the opportunity to adjust received attribute names.
