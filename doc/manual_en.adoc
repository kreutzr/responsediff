...
>
  ...
  <testSet id="Person CRUD" order="strict">
     <description>Reading person data</description> <3>
...
<3> The embedded TestSet can be described with a `description` and contains a single test.
...
<5> The `request` defines which HTTP method (`method`) is used to call the endpoint (`endpoint`), which call parameters (`parameters`) are to be appended to the endpoint URL and which headers are to be set.
