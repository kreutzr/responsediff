= ResponseDiff - Version 1.4.1-SNAPSHOT
:encoding: utf-8
:lang: de
:toc: left
:toclevels: 5
...
[NOTE]
For demonstration purposes, the file `\_custom-pdf-theme.yml` is included in the `reporter` directory upon delivery. It shows an example of how to embed a PDF theme. Because very bright colors were chosen in the example file, the file has been intentionally renamed with an underscore (`_`) so that it is not used during report generation.
...
----
<XmlResponseDiffSetup ...>
  <testSet id="Complex test" order="strict" report="success,fail,skip,warn,orga">
...
<3> The test set is (as <1>) not included in the report in detail due to the `orga="true"` flag if no `orga` entry is found in the surrounding `report` attribute (see section `<<AsciiDoc (default)>>`).
====
