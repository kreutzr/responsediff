...
* Upgrade "build-helper-maven-plugin" plugin from 3.6.1 to 3.6.2
* Upgrade "jaxb2-maven-plugin" plugin from 4.1.0 to 4.2.0
* Upgrade "properties-maven-plugin" plugin from 1.3.0 to 1.3.1
* Upgrade "xml-maven-plugin" plugin from 1.2.1 to 1.2.2
* Upgrade asciidoctorj-pdf from 2.3.23 to 2.3.27
* Upgrade jackson-databind from 2.22.2 to 2.22.3
* Upgrade mockito-core from 5.23.0 to 5.24.0
* Upgrade mockito-junit-jupiter from 5.23.0 to 5.24.0
* Upgrade slf4j from 2.0.19 to 2.0.20
...
* Upgrade "dependency-check-maven" plugin from 12.2.2 to 13.0.0
* Upgrade "license-maven-plugin" plugin from 5.1.1 to 5.1.2
* Upgrade "maven-compiler-plugin" plugin from 3.15.0 to 3.16.0
* Upgrade "maven-surefire-plugin" plugin from 3.5.6 to 3.6.0
* Upgrade jackson-databind from 2.22.1 to 2.22.2
* Upgrade junit-jupiter from 6.1.2 to 6.1.3
* Upgrade slf4j from 2.0.18 to 2.0.19
...
* Some report style fine tuning.
* Initial variables may be configured when invoking ResponseDiff.
* Allowed to skip response comparison and use ResponseDiff to trigger requests workflows only (e.g. as an Postman or Bruno alternative).
* Allowed to define sort directives for arrays of arrays (e.g., "a[0],a[1][0]").

=== External libraries

* Upgrade "cyclonedx-maven-plugin" plugin from 2.9.1 to 2.9.3
* Upgrade "dependency-check-maven" plugin from 12.2.1 to 12.2.2
* Upgrade "license-maven-plugin" plugin from 4.6 to 5.1.1
* Upgrade "maven-dependency-plugin" plugin from 3.10.0 to 3.11.0
* Upgrade "maven-surefire-plugin" plugin from 3.5.5 to 3.5.6
* Upgrade glassfish.jaxb-runtime from 4.0.7 to 4.0.9
* Upgrade jackson-databind from 2.21.3 to 2.22.1
* Upgrade junit-jupiter from 6.0.3 to 6.1.2
* Upgrade log4j from 2.25.4 to 2.26.1
...
* Added copy button in HTML report for CURL, response body and header values that start with "{" and end with "}" and thereby appear to be JSON.
* Added support for report customization (logo, colors, fonts) for HTML and PDF.

=== Bug fixes

* Added javaagent declaration to pom.xml to avoid warnings when running mockito based tests and provide compatibility for Java version 21+.

=== External libraries

* Upgrade "asciidoctor-maven-plugin" plugin from 2.2.6 to 3.2.0
* Upgrade "dependency-check-maven" plugin from 12.2.0 to 12.2.1
* Upgrade "jaxb2-maven-plugin" plugin from 3.3.0 to 4.1.0
* Upgrade "license-maven-plugin" plugin from 4.6 to 5.0.0
* Upgrade jackson-databind from 2.21.2 to 2.21.3
* Upgrade log4j from 2.25.3 to 2.25.4
...
**NOTE:** Java 17+ is required due to the upgrade of jsonpath from 2.10.0 to 3.0.0 .

=== Hints

* The configuration parameter "ticketServiceUrl" (used when invoking ResponseDiff) is marked as deprecated. Please use "ticketServiceUrls" (just add a "s") instead. The old (singular) parameter will be removed in the future with version 2.0 .

=== Features / Changes

* Introduced configuration parameter "ticketSystemUrls" to allowed referencing more than one ticket system. The different ticket system URLs appear as key value pairs inside the resulting XML file.
* User specific request headers are supported.

=== Bug fixes

* Special character "'" is masked in request as "%27".
* Fixed bug introduced in V1.3.0 . Some change messages of level ERROR did not increase the fail counter while change messages of level WARN contributed (incorrectly) to the total counter. (Intended: total = success + fail + skip)

=== External libraries

* Upgrade "build-helper-maven-plugin" plugin from 3.5.0 to 3.6.1
* Upgrade "dependency-check-maven" plugin from 12.1.9 to 12.2.0
* Upgrade "jaxb2-maven-plugin" plugin from 3.1.0 to 3.3.0
* Upgrade "license-maven-plugin" plugin from 4.3 to 4.6
* Upgrade "maven-compiler-plugin" plugin from 3.12.1 to 3.15.0
* Upgrade "maven-gpg-plugin" plugin from 3.2.7 to 3.2.8
* Upgrade "maven-javadoc-plugin" plugin from 3.11.2 to 3.12.0
* Upgrade "maven-release-plugin" plugin from 3.0.1 to 3.3.1
* Upgrade "maven-resources-plugin" plugin from 3.3.1 to 3.5.0
* Upgrade "maven-shade-plugin" plugin from 3.5.1 to 3.6.2
* Upgrade "maven-source-plugin" plugin from 3.3.1 to 3.4.0
* Upgrade "nexus-staging-maven-plugin" plugin from 1.6.8 to 1.7.0
* Upgrade "maven-surefire-plugin" plugin from 3.2.5 to 3.5.5
* Upgrade "properties-maven-plugin" plugin from 1.2.1 to 1.3.1
* Upgrade "xml-maven-plugin" plugin from 1.1.0 to 1.2.1
* Upgrade assertj from 3.27.6 to 3.27.7
* Upgrade glassfish.jaxb-runtime from 4.0.6 to 4.0.7
* Upgrade jackson-databind from 2.20.1 to 2.21.2
* Upgrade jakarta.xml.bind-api from 4.0.4 to 4.0.5
* Upgrade jsonpath from 2.10.0 to 3.0.0
* Upgrade junit-jupiter from 6.0.1 to 6.0.3
* Upgrade log4j from 2.25.2 to 2.25.3
...
* If responses of candidate and reference (and control) instances do not differ, only the candidate response is shown in the report to avoid redundancies.
* Allowed to use variables with (other) variables defined within the same "variables" XML-block.
* Allowed to test JSON structures returned within a response header.
* Added logLevel attribute to XML expectations. Differences of level WARN will not trigger a BreakOnFailureException. Exceptions have logLevel FATAL.
* Body comparisons are more tolerant by ignoring any white spaces (including line breaks).
* Allowed "orga" attribute on "test" and "testSet" XML tags in setup to optionally hide organizational processes (e.g., test preparation and tear down) in the rendered report.
* Replaced license-maven-plugin by cyclonedx-maven-plugin for SBOM creation. Replaced src/main/resources/licenses.xslt by src/main/resources/sbom.xslt accordingly.

=== Bug fixes

* Avoided tables without rows when converting XML to adoc via XSLT for tests and thereby avoided warn logs when rending adoc to PDF.
* Fixed issue that test set includes within inner test sets where not read.
* Special characters "[", "]" and " " are masked in request as "%5B", "%5D" and "%20".
* Restricted asciidoc headline levels to 5 (AsciiDoc doesnot support more levels).
* Not a bug: Replaced deprecated code (com.fasterxml.jackson.databind.JsonNode).

=== External libraries

* Upgrade "dependency-check-maven" plugin from 12.1.3 to 12.1.9
* Upgrade asciidoctorj from 3.0.0 to 3.0.1
* Upgrade asciidoctorj-pdf from 2.3.19 to 2.3.23
* Upgrade assertj from 3.27.4 to 3.27.6
* Upgrade glassfish.jaxb-runtime from 4.0.5 to 4.0.6
* Upgrade jackson-databind form 2.20.0 to 2.20.1
* Upgrade jsonpath form 2.9.0 to 2.10.0
* Upgrade junit-jupiter from 5.13.4 to 6.0.1
* Upgrade log4j from 2.25.1 to 2.25.2
* Upgrade mockito from 5.19.0 to 5.20.0
...
* Upgrade asciidoctorj from 2.5.13 to 3.0.0
* Upgrade assertj from 3.26.3 to 3.27.4
* Upgrade glassfish.jaxb-runtime from 4.0.1 to 4.0.5
