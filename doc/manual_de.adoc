...
**Skript-basierte HTTP-Workflows**

Wird der Vergleichsmodus deaktiviert, so kann ResponseDiff mit seinem Setup auch zur Abarbeitung von aufeinander aufbauenden Server-Anfragen verwendet werden. In einem Skript definierter Parameter können dabei optional als Variablen übergeben werden, was die Verwendung sehr flexibel macht. ResponseDiff kann somit als einfache und kostengünstige Alternative zu kommerziellen, UI-basierten Werkzeugen wie Postman oder Bruno eingesetzt werden. Weitere Informationen zu diesem Modus finden sich im Abschnitt <<Konfiguration>> unter den Parametern `initialVariables` und `compareResponses`.
...
=== Java
ResponseDiff benötigt Java in einer Version 17 oder höher.
...
* `ticketServiceUrl`: Optionale URL zum Ticketsystem, so dass die Ticketnummer nur noch angehangen werden muss (z.B. `http://localhost/tickets/`). Dieser Parameter ist *deprecated* und wird in Version 2.0 entfernt werden. An seine Stelle tritt der Parameter `ticketServiceUrls`.
* `ticketServiceUrls`: Optionale, Komma-separierte Auflistung der zu verwendenden Ticketsystem-URLs, so dass die Ticketnummer nur noch angehangen werden muss (z.B. `http://localhost/tickets/`). Wenn mehr als eine URL zum Einsatz kommt, müssen die URLs über einer Kennung unterschieden werden (z.B. `ticketServiceUrls:"repo1=http://localhost/tickets/repo1/,repo2=http://localhost/tickets/repo2,old=http://localhost/legacy-ticketsystem/"`). Wird bei einer URL keine Kennung angegeben, so wird intern die Kennung `default` verwendet (z.B. `ticketServiceUrls:"new=http://localhost/new-ticketsystem/,http://localhost/legacy-ticketsystem/"`). Die Kennungen können in den Testdefinitionen verwendet werden, um das zu verwendende Ticketsystem zu referenzieren (z.B. `ticketReference="new:123"`). Wird in einer `ticketReference` keine Kennung angegeben, so wird angenommen, dass die Kennung `default` gemeint ist (z.B. `ticketReference="456"`).
...
* `useLogo`: Optionales Flag, ob beim Erzeugen von Reports ein Logo verwendet werden soll (`true`) oder nicht (`false`). (default ist `false`)
...
* `maskAuthorizationHeaderInCurl`: Optionales Flag, ob Werte des `Authorization`-Headers maskiert werden sollen (`true`) oder nicht (`false`). (default ist `true`)
...
* `referenceHeaders`: Optionale Header, die beim Aufruf der Referenz-Instanz fix verwendet werden sollen. (z.B. `[ { "name" : "Authorization", "value" : "bearer ..." } ]`)
* `controlHeaders`: Optionale Header, die beim Aufruf der Kontroll-Instanz fix verwendet werden sollen. (z.B. `[ { "name" : "Authorization", "value" : "bearer ..." } ]`)
...
* `initialVariables`: Optionale Liste von Variablen, die in das TestSetup (vgl. Parameter `xmlFilePath`) übernommen werden sollen. Existierende Setup-Variablen mit gleicher `id` werden dabei für den Testlauf überschrieben. Die hier übergebenen Variablen sind in der erzeugten XML-Datei mit dem Attribut `configured` (Wert `true`) markiert.
* `compareResponses`: Optionales Flag, ob Server-Antworten vergleichen werden sollen (`true`) oder nicht (`false`). (default ist `true`)
...
* `orga`: Tests und TestSets werden per Default nicht berücksichtigt, wenn am jeweiligen Test oder TestSet das Attribut `orga` auf `true` gesetzt wurde (Default ist `false`). Dies kann interessant sein, wenn der Report nur die "echten" Tests ausweisen soll, nicht aber das ggfs. dafür notwendige Aufbauen und Abräumen des Testszenarios. Mit `orga` markierte Tests und TestSets werden (nur) in den Report eingebunden, wenn das `report`-Attribut zusätzlich `orga` enthält.
...
Eine Individualisierung des HTML-Reports kann wie in Abschnitt <<Individualisierter Report>> beschrieben vorgenommen werden.
...
Eine Individualisierung des PDF-Reports kann wie in Abschnitt <<Individualisierter Report>> beschrieben vorgenommen werden.
...
* In der Liste `x` wird nur das Element `x[1][0]` sortiert.
...
[CAUTION]
====
ResponseDiff erlaubt nicht das Überschreiben von bereits definierten Variablen durch Antwortwerte, da dies die Nachvollziehbarkeit der Aufrufe und ihrer Parameter unnötig verkomplizieren würde. Wird ein Überschreibungsversuch erkannt, so wird eine Warnmeldung ausgegeben und der Originalwert beibehalten.
====
...
[CAUTION]
Wird ein Test aufgrund des Attributs `ifExecutionContextContains` nicht ausgeführt, so können natürlich auch keine Variablenwerte aus einer Antwort gelesen werden.
...
.Test eines JSON Antwort-Headers
====
[source]
----
<?xml version="1.0" encoding="UTF-8"?>
...
=== Anwender-spezifische Request-Header

Manche Tests oder TestSets können auf spezielle Anwender mit bestimmten Rollen zugeschnitten sein. Die technische Identifikation der Anwender erfolgt über den `Authorization`-Header, der beim ResponseDiff-Aufruf über die Schlüssel `candidateHeaders`, `referenceHeaders` und `controllHeaders` im übergebenen JSON konfiguriert werden. Um die `Authorization`-Header verschiedener Anwender unterscheiden zu können, muss das optionale Attribut `userId` genutzt werden.

.Konfiguration Anwender-spezifischer Requests
====
[source]
----
{ ...,
...
* Das Attribut `userId` ist optional, solange die Tests sich nur auf einen Anwender beziehen. Sobald ein weiterer Anwender in die Tests aufgenommen wird, ist dieses Attribut mandatorisch.
* Das Attribut wird ausschließlich für Request-Header genutzt.
* Die Groß-Kleinschreibung des angegebenen Werts ist relevant.
* Die Verwendung von Variablen wird für das Attribut aktuell nicht unterstützt.

Tests und TestSets verfügen ebenfalls über ein optionales Attribut `userId`, um spezielle Anwender zu referenzieren. Die Information wird in eingekapselte Tests und TestSets vererbt, kann dabei aber auch überschrieben werden.
Ist das Attribut `userId` bei einem Test direkt oder durch Vererbung gesetzt, so werden alle Header ohne spezielle `userId` sowie alle Header mit passendem Wert bei der Anfrage verwendet.

.Verwendung von Anwender-spezifischen Requests
====
[source]
----
<?xml version="1.0" encoding="UTF-8"?>
...
----
<1> Für alle Tests, lokale und inkludierte TestSets wird der Anwender `PRIVILEGED_USER` verwendet.
<2> Dieser Test erbt das Attribut `userId` mit Wert `PRIVILEGED_USER` aus der umschließenden Struktur.
<3> Für diesen TestSet und die eingeschlossenen Tests wurde das Attribut `userId` überschrieben.
<4> Dieses TestSet erbt die `userId` mit Wert `PRIVILEGED_USER` aus der umschließenden Struktur.
====

=== Individualisierter Report

Nicht selten ist es sinnvoll, den erzeugten Report an das eigene Firmendesign anzupassen. Hierzu bietet ResponseDiff die Möglichkeit, ein Logo einzubinden, sowie Farben und Schriften anzupassen. Zudem ist es generell möglich, das XSLT-Skript, welches den zugrundeliegenden XML-Report nach AsciiDoc (`*.adoc`) transformiert, individuell zu gestalten. Die Anpassung bzgl. Farben und Schriften erfolgt im zweiten Schritt, in dem der aus XML transformierte AsciiDoc-Report in die Zielformate (z.B. HTML und PDF) konvertiert wird.

==== Einbinden eines Logos

Damit ein Logo im Report angezeigt wird, muss

* das Logo als Datei `logo.png` direkt neben der XSLT-Datei (z.B. `report-to-adoc.xslt`) liegen, die zur Reporterzeugung verwendete werden soll (vgl. `xsltFilePath` im Abschnitt "<<Konfiguration>>") und
* der Parameter `useLogo` (vgl. ebenfalls Abschnitt "<<Konfiguration>>") beim Aufruf als `true` übergeben werden.

[NOTE]
Bei der Auslieferung befindet sich im Verzeichnis `reporter` zu Demonstrationszwecken die Datei `logo.png` (das ResponseDiff-Logo). Die Übergabe des Parameters `useLogo` ist im vorbereiteten Skript `start-responseDiff` bereits vorgenommen.

==== Anpassen des Designs für HTML

Dem Report liegt initial ein Default-StyleSheet zugrunde. Um Farben und Schriften anzupassen muss eine Datei `custom-html-overrides.html` direkt neben der XSLT-Datei (z.B. `report-to-adoc.xslt`) liegen, die zur Reporterzeugung verwendete werden soll (vgl. `xsltFilePath` im Abschnitt "<<Konfiguration>>"). Der Inhalt kann beliebige CSS-Selektoren und -Deklarationen beinhalten, die ein einem klassischen `<style>`-HTML-Block zusammengefasst sind. Es gibt keine Einschränkung hinsichtlich der CSS-Verwendung.

[NOTE]
Bei der Auslieferung befindet sich im Verzeichnis `reporter` zu Demonstrationszwecken die Datei `\_custom-html-overrides.html`. Sie zeigt exemplarisch, wie eine Anpassung mit Standard CSS-Syntax realisiert werden kann. Da es sich bei der Datei um ein HTML-Fragment handelt, welches bei der Konvertierung am Ende des `<head>`-Blocks eingefügt wird, ist es prinzipiell auch möglich in der Datei einen `<script>`-Block zu platzieren und individuellen JavaScript-Code einzubetten.

==== Anpassen des Designs für PDF

Dem Report liegt initial ein Default-Theme zugrunde. Um Farben und Schriften anzupassen muss eine Datei `custom-pdf-theme.yml` direkt neben der XSLT-Datei (z.B. `report-to-adoc.xslt`) liegen, die zur Reporterzeugung verwendete werden soll (vgl. `xsltFilePath` im Abschnitt "<<Konfiguration>>"). Es ist ratsam die Zeile `extends: default` zu belassen und nur die Eigenschaften anzugeben, die überschrieben werden sollen. Prinzipiell gibt es keine Einschränkung hinsichtlich der Anpassungsmögichkeiten.

[NOTE]
Bei der Auslieferung befindet sich im Verzeichnis `reporter` zu Demonstrationszwecken die Datei `\_custom-pdf-theme.yml`. Sie zeigt exemplarisch, wie die Einbettung eines PDF-Themes realisiert werden kann. Die Syntax ist stark abhängig von der verwendeten 3rd-Party-Bibliothek AsciiDoctor-PDF.
...
----
<1> Mittels `breakOnFailure` werden Folgefehler bei der eigentlichen Testausführung vermieden, wenn dieses TestSet nicht fehlerfrei ausgeführt werden konnte.
<2> Die TestSets mit IDs `test` und `after` werden im Report als "übersprungen" markiert, wenn das Vorbereitungs-TestSet `before` nicht erfolgreich war.
<3> Das TestSet wird (wie auch Punkt 1) im Report wegen der Markierung `orga="true"` berücksichtigt, wenn sich im umschließenden `report`-Attribut kein Eintrag `orga`  findet (vgl. Abschnitt `<<AsciiDoc (default)>>`).
====

Wird die Setup-Datei zu umfangreich, können die "Aufbau"- (`before`) und "Abbau"- (`after`) TestSets in eigene Dateien ausgelagert und per `testSetInclude`-Tag eingebunden werden. Eine Auslagerung ist zudem sinnvoll, wenn die so ausgelagerten Aufrufe auch an anderer Stelle Verwendung finden sollen. Ein gewünschtes Attribut `breakOnFailure` muss bei Auslagerung in der ausgelagerten Datei gesetzt werden.
