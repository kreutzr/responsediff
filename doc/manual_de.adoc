...
[NOTE]
Bei der Auslieferung befindet sich im Verzeichnis `reporter` zu Demonstrationszwecken die Datei `\_custom-html-overrides.html`. Sie zeigt exemplarisch, wie eine Anpassung mit Standard CSS-Syntax realisiert werden kann. Da in der Beispieldatei sehr grelle Farben gewählt wurden, ist die Datei absichtlich mit einem `_` so umbenannt, dass sie bei der Reporterzeugung nicht verwendet wird. Da es sich bei der Datei um ein HTML-Fragment handelt, welches bei der Konvertierung am Ende des `<head>`-Blocks eingefügt wird, ist es prinzipiell auch möglich in der Datei einen `<script>`-Block zu platzieren und individuellen JavaScript-Code einzubetten.
...
[NOTE]
Bei der Auslieferung befindet sich im Verzeichnis `reporter` zu Demonstrationszwecken die Datei `\_custom-pdf-theme.yml`. Sie zeigt exemplarisch, wie die Einbettung eines PDF-Themes realisiert werden kann. Da in der Beispielldatei sehr grelle Farben gewählt wurden, ist die Datei absichtlich mit einem `_` so umbenannt, dass sie bei der Reporterzeugung nicht verwendet wird.
...
<3> Das TestSet wird (wie auch <1>) im Report wegen der Markierung `orga="true"` berücksichtigt, wenn sich im umschließenden `report`-Attribut kein Eintrag `orga`  findet (vgl. Abschnitt `<<AsciiDoc (default)>>`).
====
