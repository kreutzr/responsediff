...
Wird der Vergleichsmodus deaktiviert, so kann ResponseDiff auch zur Abarbeitung von aufeinander aufbauenden Server-Anfragen verwendet werden. In einem Skript definierter Parameter können dabei als Variablen übergeben werdden, was die Verwendung sehr flexibel macht. ResponseDiff kann somit als einfache und kostengünstige Alternative zu kommerziellen, UI-basierten Werkzeugen wie Postman oder Bruno eingesetzt werden. Weitere Informationen zu diesem Modus finden sich im Abschnitt <<Konfiguration>> unter den Parametern `initialVariables` und `compareResponses`.
